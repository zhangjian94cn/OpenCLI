...
function discoverAppPathForEntry(app: Pick<ElectronAppEntry, 'displayName' | 'bundleId' | 'appPath'> & { processName?: string }): string | null {
  if (process.platform !== 'darwin') {
    return null;
  }

  // Fixed app path (fork feature): trust the configured path when
  // display-name/bundle-id lookup is unreliable (e.g. Claude.app).
  if (app.appPath) {
    return app.appPath;
  }
